...
def build_style_table_all(sources):
    ...
    year_col = find_col(["년도(Now)", "년도", "연도(Now)", "연도"], df=df_base)
    df_base["_style"] = df_base[style_col].astype(str).str.strip()
    df_base["_brand"] = df_base[brand_col].astype(str).str.strip()
    def _str_strip_dot0(s):
        return pd.Series(s, index=s.index).astype(str).str.strip().str.replace(r"\.0$", "", regex=True)
    if year_col and season_col and year_col in df_base.columns and season_col in df_base.columns:
        df_base["_season"] = _str_strip_dot0(df_base[year_col]).str[:4] + _str_strip_dot0(df_base[season_col])
    else:
...
def build_inout_aggregates(io_bytes):
    ...
    sale_amt_col = find_col(["누적 판매액[외형매출]", "누적판매액", "판매액"], df=df)
    first_in_col = find_col(["최초입고일", "입고일"], df=df)
    in_qty_col = find_col(["입고량"], df=df)
    if not style_col or not brand_col:
        return [], {}, pd.DataFrame()
    season_col = find_col(["시즌", "season"], df=df)
    year_col = find_col(["년도(Now)", "년도", "연도(Now)", "연도"], df=df)
    df["_style"] = df[style_col].astype(str).str.strip()
    df["_brand"] = df[brand_col].astype(str).str.strip()
    # 브랜드·시즌별 집계를 spao_style_dashboard의 yearSeason(20262)과 동일하게 년도+시즌 조합 사용
    def _str_strip_dot0(s):
        return pd.Series(s, index=s.index).astype(str).str.strip().str.replace(r"\.0$", "", regex=True)
    if year_col and season_col and year_col in df.columns and season_col in df.columns:
        df["_season"] = _str_strip_dot0(df[year_col]).str[:4] + _str_strip_dot0(df[season_col])
    else:
